...
# -*- coding: utf-8 -*-
"""
Serveur MCP de requête aux API publiques Légifrance et Judilibre
...
**FONDS COMPATIBLES**: JORF, LODA_DATE, LODA_ETAT (DATE_PUBLICATION) | JURI, CETAT, JUFI, CONSTIT (DATE_DECISION) | KALI, CIRC, ACCO (DATE_SIGNATURE)
...
def rechercher_legifrance(
    ...
    Args:
        ...
        date_debut: Date de début pour filtres dates (YYYY-MM-DD). Défaut : None
        date_fin: Date de fin pour filtres dates (YYYY-MM-DD). Défaut : None
        page: Numéro de page. Défaut: 0
        page_taille: Résultats par page (max 50). Défaut: 20
        tri: Ordre de tri (PERTINENCE, SIGNATURE_DATE_DESC, SIGNATURE_DATE_ASC, DATE_PUBLI_DESC, DATE_PUBLI_ASC). Défaut: PERTINENCE
        operateur: Opérateur entre champs (ET, OU). Défaut: "ET"

    Returns:
        Liste de résultats avec métadonnées. Utiliser consulter_legifrance(id) pour le contenu complet.
    """
    try:
        ...
        FONDS_WITH_DATE_FILTERS = [
            "JORF", "LODA_DATE", "LODA_ETAT", "JURI", "CETAT",
            "JUFI", "CONSTIT", "KALI", "CIRC", "ACCO"
        ]

        if (date_debut or date_fin) and fond not in FONDS_WITH_DATE_FILTERS:
            warning = [
                f"⚠️ ATTENTION: Les filtres de dates sont ignorés pour le fond '{fond}'. "
...
def rechercher_jurisprudence_judilibre(
    ...
    Args:
        ...
        juridiction: Code juridiction (cc, ca, tj, tcom). Défaut : None (toutes les juridictions)
        localisation: Code siège (ex: "ca_paris", "tj06088")
        chambre: Code chambre (pl, mi, civ1, civ2, civ3, comm, soc, cr)
        type_decision: Type de décision (arret, ordonnance, qpc, saisie)
        theme: Code matière juridique
        solution: Solution (cassation, rejet, annulation, etc.)
        date_debut: Date début ISO (ex: 2023-01-15)
        date_fin: Date fin ISO (ex: 2023-12-15)
    ...
    Returns:
        Liste de décisions. Utiliser consulter_decision_judilibre(id) pour le texte complet.
    """
    try:
        if judilibreapi is None:
            logger.error("API Judilibre non initialisée")
        ...
        jurisdiction_list = [juridiction] if juridiction else ["cc", "ca", "tj", "tcom"]
...
def consulter_decision_judilibre(decision_id: str) -> Any:
    ...
    """
    logger.debug(f"APPEL: consulter_decision_judilibre(decision_id='{decision_id}')")
